...
const r = Router();
r.use(authenticate);
r.get("/departments", authorize("hr", "view"), c.departments);
r.post("/departments", authorize("hr", "manage_employees"), c.createDepartment);
r.get("/job-titles", authorize("hr", "view"), c.jobTitles);
r.post("/job-titles", authorize("hr", "manage_employees"), c.createJobTitle);
