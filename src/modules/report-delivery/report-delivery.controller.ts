...
const subscription = z
  ...
  .refine(
    (v) =>
      v.channel !== "EMAIL" || z.string().email().safeParse(v.recipient).success,
    { message: "recipient must be a valid email for the EMAIL channel", path: ["recipient"] },
  )
  .refine(
    ...
    { message: "recipient must be a phone number in international format for WHATSAPP", path: ["recipient"] },
  );
...
export const reportDeliveryController = {
  ...
  create: asyncHandler(async (req: Request, res: Response) => {
    ...
    const created = await r.create(companyId(req), req.auth!.userId, parsed.data);
  ...
  update: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!parsed.success) return fail(res, "Invalid input", 422, parsed.error.flatten());
    const updated = await r.update(Number(req.params.id), companyId(req), parsed.data);
  ...
  sendLogs: asyncHandler(async (req: Request, res: Response) => {
    ok(res, await r.sendLogsForSubscription(Number(req.params.id), companyId(req)));
  }),
};
