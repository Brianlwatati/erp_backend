...
import { companyProvisionedSchema, userCreatedSchema } from "./provisioning.validators.js";
...
export const provisioningController = {
  onCompanyProvisioned: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (await provisioningRepository.hasProcessed("company_provisioned", companyId, null)) {
      return ok(res, null, "Already processed");
    }

    await provisioningRepository.seedDefaultsForCompany(companyId);
    await provisioningRepository.markProcessed("company_provisioned", companyId, null);
  ...
  onUserCreated: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (await provisioningRepository.hasProcessed("user_created", companyId, userId)) {
      return ok(res, null, "Already processed");
    }

    await provisioningRepository.assignDefaultRoleToUser(userId, companyId);
    await provisioningRepository.markProcessed("user_created", companyId, userId);
