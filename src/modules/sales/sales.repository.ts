...
import { SalesOrderItem } from "./sales.types.js";
export const salesRepository = {
  list: (c: number, status?: string) =>
    query(
      `SELECT o.*,c.name AS "customerName",w.name AS "warehouseName" FROM erp_sales_orders o 
      JOIN erp_contacts c ON c.id=o.customer_id JOIN erp_warehouses w ON w.id=o.warehouse_id 
      WHERE o.ias_company_id=$1 AND ($2::varchar IS NULL OR o.status=$2) ORDER BY o.created_at DESC`,
      [c, status ?? null],
    ),
  find: (id: number, c: number) =>
    queryOne(
      `SELECT o.*,c.name AS "customerName",w.name AS "warehouseName" FROM erp_sales_orders o 
      JOIN erp_contacts c ON c.id=o.customer_id JOIN erp_warehouses w ON w.id=o.warehouse_id 
      WHERE o.id=$1 AND o.ias_company_id=$2`,
      [id, c],
    ),
  items: (id: number) =>
    query(
  ...
  create: async (c: number, u: number, x: any) =>
    withTransaction(async (client) => {
      ...
      const customer = await client.query(
        `SELECT id FROM erp_contacts WHERE id=$1 AND ias_company_id=$2 AND contact_type 
        IN ('CUSTOMER','BOTH')`,
        [x.customerId, c],
      );
      if (!customer.rowCount) throw new Error("Customer not found");
      const wh = await client.query(
        `SELECT id, name FROM erp_warehouses WHERE id=$1 AND ias_company_id=$2 AND status='ACTIVE'`,
        [x.warehouseId, c],
      );
      if (!wh.rowCount) throw new Error("Warehouse not found");
      let subtotal = 0,
        tax = 0;
      const prepared: SalesOrderItem[] = [];
      for (const i of x.items) {
        const p = await client.query(
          `SELECT id,sku,name,sell_price FROM erp_products WHERE id=$1 AND ias_company_id=$2 
          AND status='ACTIVE'`,
          [i.productId, c],
        );
      ...
      // console.log("Prepared data for sales order insertion:", arrydata); // Debugging line
      const o = await client.query(
        `INSERT INTO erp_sales_orders(ias_company_id,order_number,customer_id,warehouse_id, 
        warehouse_name, status,
        currency,subtotal,discount_amount,tax_amount,total_amount,notes,created_by) 
    ...
    withTransaction(async (client) => {
      ...
      for (const i of items.rows) {
        const s = await client.query(
          `SELECT quantity,reserved_quantity FROM erp_stock_levels WHERE product_id=$1 AND warehouse_id=$2 
          FOR UPDATE`,
  ...
  ship: async (id: number, c: number, u: number) =>
    withTransaction(async (client) => {
      ...
      for (const i of items.rows) {
        ...
        await client.query(
          `INSERT INTO erp_stock_movements(ias_company_id,product_id,product_sku,product_name,
          warehouse_id,warehouse_name,quantity_delta,unit_cost,reason,reference_type,reference_id,created_by) 
          VALUES($1,$2,$3,$4,$5,$6,$7,$8,'SALE','sales_order',$9,$10)`,
          [
            c,
            i.product_id,
            i.product_sku,
